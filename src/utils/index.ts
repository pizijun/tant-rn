...
export function parseFormat(format: string, timeData: TimeData): string {
  ...
  if (format.indexOf('DD') === -1) {
    ...
    format = format.replace('DD', padStart(days));
  }

  if (format.indexOf('HH') === -1) {
    minutes += hours * 60;
  } else {
    format = format.replace('HH', padStart(hours));
  }

  if (format.indexOf('mm') === -1) {
    seconds += minutes * 60;
  } else {
    format = format.replace('mm', padStart(minutes));
  }

  if (format.indexOf('ss') === -1) {
    milliseconds += seconds * 1000;
  } else {
    format = format.replace('ss', padStart(seconds));
  }

  if (format.indexOf('S') !== -1) {
    const ms = padStart(milliseconds, 3);
